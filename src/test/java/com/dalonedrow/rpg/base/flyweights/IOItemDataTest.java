...
public class IOItemDataTest {
	...
	public void canSetFields() throws RPGException {
		assertEquals("price is 0", data.getPrice(), 0);
		assertEquals("weight is 0", data.getWeight(), 0);
		assertNull("desc is null", data.getDescription());
		assertNull("ioe is null", data.getEquipitem());
		data.setPrice(12);
		data.setWeight(3);
		data.setDescription(new char[] { 't', 'e', 's', 't' });
		data.setEquipitem(ioe);
		data.setItemName(new char[] { 't', 'e', 's', 't' });
		assertEquals("price is 12", data.getPrice(), 12);
		assertEquals("weight is 3", data.getWeight(), 3);
