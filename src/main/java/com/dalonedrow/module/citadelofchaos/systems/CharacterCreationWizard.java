...
public final class CharacterCreationWizard {
	...
	private CitadelPC		newCharacter;
	...
	public void rollNewCharacter() throws RPGException {
		if (newCharacter == null) {
			newCharacter = new CitadelPC();
		}
		newCharacter.setBaseAttributeScore("SK",
				Dice.D6 + Diceroller.getInstance().rolldX(Dice.D6));
		newCharacter.setBaseAttributeScore("ST",
				2 * Dice.D6 + Diceroller.getInstance().rollXdY(2, Dice.D6));
		newCharacter.setBaseAttributeScore("LK",
				Dice.D6 + Diceroller.getInstance().rolldX(Dice.D6));
		newCharacter.setBaseAttributeScore("MSK",
				newCharacter.getBaseAttributeScore("SK"));
		newCharacter.setBaseAttributeScore("MST",
				newCharacter.getBaseAttributeScore("ST"));
		newCharacter.setBaseAttributeScore("MLK",
				newCharacter.getBaseAttributeScore("LK"));
	}
	/**
		...
		if (archetype != newCharacter.getProfession()) {
			...
			if (ptsNeeded <= pcRace.getStartingEp()) {
				...
				System.out.println("start dx::"
						+ newCharacter.getBaseAttributeScore("DX"));
				System.out.println("start iq::"
						+ newCharacter.getBaseAttributeScore("IQ"));
				// set starting spells
