...
import com.dalonedrow.rpg.graph.IndexMinPQ;
 ...
 *
 */
public final class RiverDijkstraSearch {
	 ...
	 * <tt>V</tt> - 1
	 */
    public RiverDijkstraSearch(final EdgeWeightedDirectedGraph graph,
    		final int source) throws RPGException {
    	if (!HexMap.getInstance().hasRiverNode(source)) {
    		throw new RPGException(ErrorMessage.BAD_PARAMETERS, 
    				"not a valid river node");
    	}
    	if (HexMap.getInstance().getRiverNodeById(source).getType() 
    			!= RiverGraphNode.RIVER_BANK) {
    		throw new RPGException(ErrorMessage.BAD_PARAMETERS, 
    				"cannot start river path from river itself; "
    				+ "must be a river bank");
    	}
