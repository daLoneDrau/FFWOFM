...
public class TextProcessor {
		...
		try {
			...
			if (string.length() > innerWidth) {
				...
				for (int i = 0, len = split.length; i < len; i++) {
					...
					sb.append(split[i]);
					for (int j = innerWidth - left - split[i].length(); j > 0; j--) {
						sb.append(' ');
					}
					if (i + 1 < len) {
						sb.append('\n');
	 ...
	 */
	public String getSelectionsAsColumns(final String[] cols,
			final String seperator, final boolean bordered) throws RPGException {
	...
	public String processText(final BaseInteractiveObject playerIO,
			...
			if (targetIO.hasIOFlag(IoGlobals.IO_03_NPC)) {
				tokenized =
						tokenized.replaceAll("<tioName>", new String(targetIO
								.getNPCData().getName()));
			} else if (targetIO.hasIOFlag(IoGlobals.IO_14_PARTY)) {
				tokenized = tokenized.replaceAll("<tioName>", new String(
						((BpIoGroup) targetIO).getName()));
		...
		if (tokenized.indexOf("<tioNameApos>") > -1) {
			...
			try {
				...
				if (targetIO.hasIOFlag(IoGlobals.IO_03_NPC)) {
					...
					name = new String(((BpIoGroup) targetIO).getName());
				} else if (targetIO.hasIOFlag(IoGlobals.IO_02_ITEM)) {
		...
		if (tokenized.indexOf("<tioGroupApos>") > -1) {
			PooledStringBuilder sb =
					StringBuilderPool.getInstance().getStringBuilder();
			String tName = ((BpIoGroup) targetIO).getName();
			try {
				sb.append(tName);
				if (tName.toLowerCase().charAt(sb.length() - 1) == 's'
		...
		if (tokenized.indexOf("<tioGroup>") > -1) {
			tokenized = tokenized.replaceAll(
					"<tioGroup>", ((BpIoGroup) targetIO).getName());
		}
