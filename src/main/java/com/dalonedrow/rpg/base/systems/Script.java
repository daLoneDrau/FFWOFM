...
STACKED extends StackedEvent<IO>> {
		...
		if (gvars != null) {
			for (int i = gvars.length - 1; i >= 0; i--) {
				if (gvars[i] != null
						...
						|| gvars[i].getType() == ScriptConsts.TYPE_L_08_TEXT)
			...
			for (; i >= 0; i--) {
				if (io.getScript().getLocalVariable(i) != null
						&& (io.getScript().getLocalVariable(i).getType()
								== ScriptConsts.TYPE_G_00_TEXT
						|| io.getScript().getLocalVariable(i).getType()
							== ScriptConsts.TYPE_L_08_TEXT)
						&& io.getScript().getLocalVariable(i).getText()
							!= null) {
	...
	/**
	 * Sends an initialization event to an IO.  The initialization event runs
	 * the local script first, followed by the over script.
	 * @param io the IO
	 * @return {@link int}
	 * @throws RPGException if an error occurs
	 */
		...
		if (countTimers() > 0) {
			for (int i = 0, len = this.maxTimerScript; i < len; i++) {
				...
				if (timer.exists()) {
					...
					System.out.println((timer.getLastTimeCheck() + timer.getCycleLength()));
